...
test('live sample then finalized message: message wins by seq (never double counted)', () => {
  ...
  const message: ConversationViewNode = stepNode('0:0:step', {
    turn: 0, step: 0,
    samples: [sample(25, peakMs, { model: 'deepseek-flash', usage: usage({ inputTokens: 1_000_000, outputTokens: 500_000 }) })],
  })
  ...
  assert.equal(readout.totalCny, 2.0 + 0.5 * 8.0)
  ...
  const readout = b.replace({
    nodes: [stepNode('0:0:step', {
      ...
      samples: [
        ...
        sample(10, peakMs, { model: 'deepseek-flash', usage: usage({ inputTokens: 1_000_000, outputTokens: 0 }) }),
        // Final settlement: sample after the marker (generation 1).
        sample(25, valleyMs, { model: 'deepseek-flash', usage: usage({ inputTokens: 2_000_000, outputTokens: 0 }) }),
      ],
    })],
    timeline: {} as never,
  })
  // gen0: 1M at peak = 2.0; gen1: 2M at valley = 1.0 × 2 = 2.0; total 4.0.
  assert.equal(readout.totalCny, 4.0)
  assert.equal(readout.recordCount, 2)
  assert.equal(readout.peakCny, 2.0)
  assert.equal(readout.valleyCny, 2.0)
})
...
test('attempt without own model takes the nearest preceding header node', () => {
  ...
  assert.equal(readout.totalCny, 2.0 + 0.5 * 8.0)
  assert.equal(readout.recordCount, 1)
  assert.deepEqual(readout.byModel, [{ model: 'deepseek-flash', totalCny: readout.totalCny, recordCount: 1 }])
})
...
test('upsert carrying the accumulated state keeps the newest sample per generation', () => {
  ...
  const readout = b.replace({
    nodes: [stepNode('0:0:step', {
      turn: 0, step: 0,
      samples: [
        sample(10, peakMs, { model: 'deepseek-flash', usage: usage({ inputTokens: 900_000, outputTokens: 400_000 }) }),
        sample(25, peakMs, { model: 'deepseek-flash' }),
      ],
    })],
    timeline: {} as never,
  })
...
test('mixed windows split peak/valley and report valley saving', () => {
  ...
  const readout = b.replace({
    nodes: [
      stepNode('0:0:step', { turn: 0, step: 0, samples: [sample(1, peakMs, { model: 'deepseek-flash' })] }),
      stepNode('0:1:step', { turn: 0, step: 1, samples: [sample(2, valleyMs, { model: 'deepseek-flash' })] }),
    ],
    timeline: {} as never,
  })
  // peak: 1M×2 + 500K×8 = 6.0; valley half = 3.0; total 9.0; valleySaved = 3.0.
  assert.equal(readout.peakCny, 6.0)
  assert.equal(readout.valleyCny, 3.0)
  assert.equal(readout.totalCny, 9.0)
  assert.equal(readout.valleySavedCny, 3.0)
})
...
test('replace resets prior accumulation (window rebuild)', () => {
  ...
  b.replace({
    nodes: [stepNode('0:0:step', {
      turn: 0, step: 0,
      samples: [sample(1, peakMs, { model: 'deepseek-flash' })],
    })],
    timeline: {} as never,
  })
