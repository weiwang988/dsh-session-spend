...
test('retried attempt and final message are both billed at their own instants', () => {
  const summary = computeCost(foldCostItems([
    { kind: 'header', seq: 5, model: 'deepseek-v4-flash' },
    record(10, { turn: 0, step: 0, time: at(28, 10), usage: { inputTokens: 1_000_000, outputTokens: 0 } }),
    { kind: 'retry', seq: 15, turn: 0, step: 0 },
    record(30, {
      turn: 0, step: 0, time: at(28, 11), model: 'deepseek-v4-flash',
      usage: { inputTokens: 2_000_000, outputTokens: 0 },
    }),
  ]), officialPriceTable)
  // Failed attempt 1M (peak, flash via header) + final message 2M (peak): 3.0 + 6.0.
  assert.equal(summary.totalCny, 9.0)
  assert.equal(summary.recordCount, 2)
})

test('attempt without own model takes the routed header model', () => {
  const summary = computeCost(foldCostItems([
    { kind: 'header', seq: 1, model: 'deepseek-v4-flash' },
    record(10, { turn: 0, step: 0, time: at(28, 10), usage: { inputTokens: 1_000_000, outputTokens: 0 } }),
  ]), officialPriceTable)
  assert.equal(summary.totalCny, 3.0)
  assert.equal(summary.recordCount, 1)
  assert.deepEqual(summary.byModel, [{ model: 'deepseek-v4-flash', totalCny: 3.0, recordCount: 1 }])
})
